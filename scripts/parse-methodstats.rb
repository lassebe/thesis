...
input_file = File.read('sample.json')

#JSON.parse(input_file)
stats = JSON.parse(STDIN.gets)

#interrupting_methods = Hash.new(0)
puts "name\trecorded\ttotal calls\tcontext match\tratio (match/total)\tinterrupting native call"

stats["methodStats"].sort_by { |s| [s["counter"]["recorded"] ? 1 : 0, s["counter"]["totalCalls"]] }.reverse.each do |methodStats|
